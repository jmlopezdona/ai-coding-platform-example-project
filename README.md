...
2. The Skaffold sidecar renders `$VAR` placeholders in `skaffold.yaml` and runs `skaffold dev`
3. Skaffold builds images via Kaniko (in-cluster), pushes to ECR, and deploys the stack using `helm/stack-chart`
...
These variables are rendered by the platform before Skaffold starts:
...
All development happens inside the platform's EKS cluster. The Skaffold sidecar
in the dev pod handles building, deploying, and syncing automatically.

```bash
# Create an environment (from the platform repo)
envctl create --dc-id dc-42 --dc-branch dc/feature-auth \
  --repo-url https://github.com/your-org/example-project.git
```
